...
public class StmtCodeGenerator extends Visitor<Void> {
	...
	public Void visitBreakStmt(BreakStmt nd) {
		...
		WhileStmt parent = nd.getEnclosingLoop(); // breaks appear in a while, get the while stmt that encloses this break
		Unit exit = breakTargets.get(parent); // find where to go to after the break statment
		units.add(j.newGotoStmt(exit)); // add a goto stmt for a break, and go to the exit
	...
	public Void visitWhileStmt(WhileStmt nd) {
		...
		NopStmt exit = j.newNopStmt();
		
		breakTargets.put(nd,  exit); // update hashmap to keep track of each while stmt and their corresponding exit
		units.add(start);
		
		Value condition = ExprCodeGenerator.generate(nd.getExpr(), fcg); // generate condition stmt
		units.add(j.newIfStmt(j.newEqExpr(condition,  IntConstant.v(0)), exit)); // if condition == false, go to exit
		nd.getBody().accept(this);
		
		units.add(j.newGotoStmt(start)); // if we reach the end of the loop, go back to the start
		units.add(exit); // outside of the loop
